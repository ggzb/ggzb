...
public class PopupWindowUtil {
   ...
   public PopupWindowUtil(View anchor) {
      ...
      this.windowManager =
          (WindowManager) this.anchor.getContext().getSystemService(Context.WINDOW_SERVICE);
      onCreate();
   }

   /**
    * Anything you want to have happen when created. Probably should create reset
   ...
   private void preShow() {
      if (this.root == null) {
         throw new IllegalStateException("setContentView was not called with reset view to display.");
      }
      onShow();
   ...
   private void dimBackground(float bgDim, float bgAlpha) {
      WindowManager.LayoutParams lp =
          ((Activity) this.anchor.getContext()).getWindow().getAttributes();
      lp.alpha = bgAlpha;
      lp.dimAmount = bgDim;
      ((Activity) this.anchor.getContext()).getWindow().setAttributes(lp);
      ((Activity) this.anchor.getContext()).getWindow()
   ...
   /**
    * Will inflate and set the view from reset resource id
    */
   ...
   /**
    * Displays like reset popdown menu from the anchor view
    * 在anchor view的左下方弹出
    */
   ...
   /**
    * Displays like reset QuickAction from the anchor view.
   ...
   public void showCenter() {
     ...
     * 与父控件右对齐在其下方显示
     */
   public void showAlignParentRight(int xOffset, int yOffset) {
      this.preShow();
//      this.window.setAnimationStyle(R.style.GrowFromBottom);
...
//         Trace.d("**>发现popwindow高度不够");
         yPos = location[1] + anchor.getHeight();
         this.window.setAnimationStyle(R.style.GrowFromTop);
//      }else {
//         yPos = location[1] - this.root.getHeight();
//      }

      int xPos = anchor.getWidth() / 2 - rootWidth;
      this.window.showAtLocation(this.anchor, Gravity.NO_GRAVITY, (location[0]+anchor.getWidth()/2) + xPos + xOffset,
              yPos + yOffset);
   }
   ...
   public void showBottomWithAlpha() {
      ...
      this.window.showAtLocation(this.anchor, Gravity.BOTTOM | Gravity.LEFT, 0,
   ...
   public void showTop() {
      ...
      int result = 0;
      int resourceId = this.anchor.getContext()
          .getResources()
