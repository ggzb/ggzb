...
import dammu.utils.DpOrSp2PxUtil;
...
public class DanmuControl {
   ...
   private static final long ADD_DANMU_TIME = 6;
   ...
   private int BITMAP_WIDTH = 28;//头像的大小
...
//        EMOJI_SIZE = DpOrSp2PxUtil.dp2pxConvertInt(context, EMOJI_SIZE);
   ...
   private void initDanmuConfig() {
      ...
      mDanmakuContext.setDanmakuStyle(IDisplayer.DANMAKU_STYLE_NONE)
          .setDuplicateMergingEnabled(false).setScrollSpeedFactor(1.2f)//越大速度越慢
...
//            }
   ...
   public void addDanmu(Bitmap danmu_bitmap, UserBarrageEntity danmu) {
      BaseDanmaku danmaku =
          mDanmakuContext.mDanmakuFactory.createDanmaku(BaseDanmaku.TYPE_SCROLL_RL);

      ImageSpan imgSpan = new ImageSpan(mContext, danmu_bitmap);
      SpannableString spanString = new SpannableString("icon");
      spanString.setSpan(imgSpan, 0, 4, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);

      //可能AvActivity被销毁后还有消息创建弹幕
      if (danmaku==null||mDanmakuView == null) {
         return;
      }
      danmaku.userId = Integer.parseInt(danmu.uid);
      danmaku.isGuest = false;//isGuest此处用来判断是赞还是评论
      danmaku.text = spanString;
