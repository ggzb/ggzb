 ...
 * 频道管理
 */
public class ShiHuoStep2Activity extends BaseActivity implements
...
//		setTranslucentStatus() ;
	...
	private void setSkills(String skills, String jobs) {
		...
		RequestInformation request = new RequestInformation(UrlHelper.URL_HEAD
				+ "/reg/job_edit", RequestInformation.REQUEST_METHOD_POST);
		request.addPostParams("skill", skills);
		...
		request.setCallback(new JsonCallback<UserInfo>() {
			...
			public void onCallback(UserInfo callback) {
				...
				if (callback.getStat() == 200) {
					...
					if (back_home) {
						ShiHuoStep2Activity.this.finish();
					} else {
						Intent intent = new Intent(ShiHuoStep2Activity.this,
								ShiHuoStep3Activity.class);
