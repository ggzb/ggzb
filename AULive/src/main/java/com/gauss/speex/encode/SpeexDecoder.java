...
public class SpeexDecoder {
	...
	private SpeexCompletionListener listener;
	...
	public void setListener(SpeexCompletionListener listener) {
		...
		try {
			...
			while (true) {
				if (Thread.interrupted()) {
					// Trace.d("voice Thread.interrupted() :"
					// + Thread.interrupted());
					dis.close();
					track.stop();
					return;
				...
				if (this.isPaused()) {
					// Trace.d("voice isPaused :" + isPaused());
					dis.close();
					track.stop();
					return;
				}
				...
				// make sure its reset OGG header
				if (!OGGID.equals(new String(header, 0, 4))) {
					// Trace.d("missing ogg id!");
					return;
				}
				...
				for (curseg = 0; curseg < segments; curseg++) {
					if (Thread.interrupted()) {
						dis.close();
						track.stop();
						// Trace.d("for voice Thread.interrupted() :"
						// + Thread.interrupted());
						return;
					}

					if (this.isPaused()) {
						// Trace.d("for voice isPaused :" + isPaused());
						track.stop();
						Thread.sleep(100);
						return;
					}
					...
					if (bodybytes == 255) {
						// Trace.d("sorry, don't handle 255 sizes!");
						return;
					}
					...
					// Trace.d("chksum:" + chksum + " origchksum:" + origchksum
					// + " curseg:" + curseg + " segments:" + segments);
				}
	...
	private boolean readSpeexHeader(final byte[] packet, final int offset,
		...
		// Trace.d("mode=" + mode + " sampleRate==" + sampleRate + " channels="
		// + channels + "nframes=" + nframes + "framesize=" + frameSize);
	...
	public interface SpeexCompletionListener {
		void onCompletion();
	}
}
